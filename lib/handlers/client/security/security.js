...
SecurityClientHandler.prototype.send = function(ctx, callback) {
    ...
    , security = utils.appendElement(doc, header, consts.security_ns, consts.security_prefix + ":Security", null)
...
SecurityClientHandler.prototype.AddNamespaces = function(doc) {	
  doc.firstChild.setAttribute("xmlns:" + consts.security_utility_prefix, consts.security_utility_ns)
  var security = select(doc, "/*[local-name(.)='Envelope']/*[local-name(.)='Header']/*[local-name(.)='Security']")[0]
  security.setAttribute("xmlns:" + consts.security_prefix, consts.security_ns)
}

SecurityClientHandler.prototype.AddTimestamp = function(doc, security) {		
  var timestamp = utils.appendElement(doc, security, consts.security_utility_ns, consts.security_utility_prefix + ":Timestamp", null)
    , created_time = new Date()
    , expires_time = new Date(created_time)
    , expires_timespan = 5
	expires_time.setMinutes ( created_time.getMinutes() + expires_timespan )
	var created = utils.appendElement(doc, timestamp, consts.security_utility_ns, consts.security_utility_prefix + ":Created", dateFormat(created_time, "isoUtcDateTime"))
	  , expires = utils.appendElement(doc, timestamp, consts.security_utility_ns, consts.security_utility_prefix + ":Expires", dateFormat(expires_time, "isoUtcDateTime"))
}
...
UsernameToken.prototype.applyMe = function(doc, security) {	
  var security = select(doc, 
    ...
    , token = utils.appendElement(doc, security, security.security_ns, consts.security_prefix + ":UsernameToken", null)
    , username = utils.appendElement(doc, token, security.security_ns, consts.security_prefix + ":Username", 
        this.options.username)
    if (this.options.password !== undefined){
      password = utils.appendElement(doc, token, security.security_ns, consts.security_prefix + ":Password", 
...
X509BinarySecurityToken.prototype.extractBase64Key = function(key) {  
  ...
  var end = key.length - "-----END PRIVATE KEY-----".length - 1
  var res = key.substring(start, end-1)  
  res = res.replace(/(\r\n|\n|\r)/gm,"")  
  return res
}
...
X509BinarySecurityToken.prototype.applyMe = function(doc, security) {    
  ...
  var security = select(doc, 
      "/*[local-name(.)='Envelope']/*[local-name(.)='Header']/*[local-name(.)='Security']")[0]
    , token = utils.appendElement(doc, security, security.security_ns, consts.security_prefix + ":BinarySecurityToken", base64Key)
  token.setAttribute("ValueType", 
        "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509v3")
  token.setAttribute("EncodingType", 
        "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary")
  token.setAttribute(consts.security_utility_prefix + ":Id", this.id)  
  return doc
}
